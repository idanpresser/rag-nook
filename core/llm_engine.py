...
class LMStudioHermesClient:
    ...
    def summarize_image(self, image_path: Union[str, Path], prompt: Optional[str] = None) -> str:
        """Sends a local image (AVIF or other formats) to the local Gemma-4 VLM endpoint to summarize/describe it.

        Args:
            image_path: Path to the image file.
            prompt: Optional custom text query for the VLM model.

        Returns:
            The text description/summary generated by the model.
        """
        ...
        # Determine mime type based on suffix
        ...
        if prompt is None:
            prompt = "Provide a concise summary and list key visual elements in this image."
        ...
        try:
            ...
            return f"VLM Image analysis failed. Error: {str(e)}"
